...
public class JsCallJava {
    ...
    private  HostJsScope hostJsScope;

    public JsCallJava (String injectedName, HostJsScope hostJsScope) {
        try {
            if (TextUtils.isEmpty(injectedName)) {
                throw new Exception("injected name can not be null");
            }
            mInjectedName = injectedName;
            this.hostJsScope = hostJsScope;
            mMethodsMap = new HashMap<String, Method>();
            //获取自身声明的所有方法（包括public private protected）， getMethods会获得所有继承与非继承的方法
            Method[] methods = this.hostJsScope.getClass().getDeclaredMethods();
    ...
    public String call(WebView webView, String jsonStr) {
        if (!TextUtils.isEmpty(jsonStr)) {
            try {
                ...
                return getReturn(jsonStr, 200, currMethod.invoke(hostJsScope, values));
            } catch (Exception e) {
                //优先返回详细的错误信息
                if (e.getCause() != null) {
    ...
    private String getReturn (String reqJson, int stateCode, Object result) {
        ...
        if (result == null) {
            ...
            insertRes = GsonUtils.toJson(result);
        } else {  //数字直接转化
